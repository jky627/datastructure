...
public:
	...
	void insert(int r, int c, int val) { //把兩個陣列放進新的陣列中
		...
		{
			//用data的第0列存入，原陣列的列數、行數、非0的值的數量///1
			data[len][0] = r;
			data[len][1] = c;
			data[len][2] = val;

			data[0][2] = len;//因為你會一直在加val
	...
	void threeCol_plus(sparse_matrix a, sparse_matrix b) {
		...
		for (i = 1, j = 1; i < a.len/*a條件式*/ && j < b.len/*b條件式*/;) { //->插入C的條列
			...
			//計算a第i列和b第j列的編號位置
			int pos_a = a.data[i][0] * a.data[0][1] + a.data[i][1];///就是第幾列資料
			int pos_b = b.data[j][0] * b.data[0][1] + b.data[j][1];///我們是一列一列去看的
			...
			則插入 a第i列的列、行，a+b的值*/
				...
				if (a.data[i][2] + b.data[j][2] != 0) {
					insert(a.data[i][0], a.data[i][1], a.data[i][2] + b.data[j][2]);///同一個位置所以沒有關係，後面的值是兩個相加
				}
				i++;
				j++;
			}
